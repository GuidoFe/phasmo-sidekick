...
module.exports = {
    ...
    buildChallengeList: function (challengeCodes, challenges) {
        var rowsToPrint = [];
        for (var _i = 0, challengeCodes_1 = challengeCodes; _i < challengeCodes_1.length; _i++) {
            code = challengeCodes_1[_i];
    ...
    getMessageArguments: function (message) {
        return message.content.split(' ').filter(function (value, _index, _arr) {
            return value != '';
        });
    },
    randomVibrantColor: function () {
        var h = Math.floor(Math.random() * 36) * 10;
        return Color.hsl(h, 100, 70).hex();
    },
    formatObjectAsList: function (ob) {
        var msg = '';
        for (var _i = 0, _a = Object.keys(ob); _i < _a.length; _i++) {
            e = _a[_i];
            msg += '- ' + e + '\n';
        }
        return msg.slice(0, -1);
